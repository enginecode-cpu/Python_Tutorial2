...
# Future 동시성
# 비동기 작업 실행
# 지연시간(Block), CPU 및 리소스 낭비 -> (File)Network I/O 관련 작업 동시성 활용 권장
...
# 구글에서 Python GIL(Global Interpreter Lock)
# GIL은 한 번에 하나의 스레드만 수행 할 수 있도록 인터프리터에 락을 거는 것
...
def main(separate_many):
    ...
    # 결과 건수
    # ProcessPoolExecutor : GIL 우회, 변경 -> os.cpu_count()
    ...
    print(msg.format(list(result_cnt), end_time))
